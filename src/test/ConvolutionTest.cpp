...
#include "torch/fb/fbcunn/layers/test/InputCentricConvolution_UpdateOutput.cuh"
#include "torch/fb/fbcunn/layers/test/CudaConvolutionTest.cuh"
#include "torch/fb/fbcunn/layers/test/ReferenceConvolutions.h"
...
class FFTTest : public ConvolutionModule {
 public:
  void updateOutput(
    Tensor<float>& input,
    Tensor<float>& filters,
    ...
    auto inputCuda = copyToCuda(input);
    auto filtersCuda = copyToCuda(filters);
    auto outputCuda = makeTHCudaTensorSameSize(output);

    fftUpdateOutput(THCudaTensor_data(state, inputCuda.get()),
                    THCudaTensor_data(state, filtersCuda.get()),
                    THCudaTensor_data(state, outputCuda.get()),
                    input.size(0), input.size(1),
                    filters.size(0),
                    input.size(2), input.size(3),
                    filters.size(2), filters.size(3),
                    makeBuffer(input, output).get());

    output = copyFromCuda(outputCuda.get());
  }
  ...
  void updateGradInput(
    ...
    fftUpdateGradInput(THCudaTensor_data(state, outputCuda.get()),
                       THCudaTensor_data(state, filtersCuda.get()),
                       THCudaTensor_data(state, inputCuda.get()),
                       input.size(0), input.size(1),
                       filters.size(0),
                       input.size(2), input.size(3),
                       filters.size(2), filters.size(3),
    ...
    input = copyFromCuda(inputCuda.get());
  }
  ...
  void accGradParameters(
    ...
    Tensor<float>& filters) override {
    ASSERT_FALSE(inputPadding); // padding not supported
    ASSERT_EQ(1.0f, scale); // scale not yet supported

    auto inputCuda = copyToCuda(input);
    auto outputCuda = copyToCuda(output);
    auto filtersCuda = makeTHCudaTensorSameSize(filters);

    fftAccGradParameters(THCudaTensor_data(state, inputCuda.get()),
                         THCudaTensor_data(state, outputCuda.get()),
                         THCudaTensor_data(state, filtersCuda.get()),
                         input.size(0), input.size(1),
                         filters.size(0),
    ...
    filters = copyFromCuda(filtersCuda.get());
  }

 private:
  // Create the GPU memory buffer that the FFT code expects
  static unique_ptr<cuComplex, CudaDeleter>
  makeBuffer(const Tensor<float>& input, const Tensor<float>& filters) {
    const auto bufferSize =
      fftGetMinBufferSize(input.size(0), input.size(1),
                          filters.size(0),
                          input.size(2), input.size(3),
                          filters.size(2), filters.size(3));
    return cudaAlloc<cuComplex>(bufferSize);
  }
};
...
class InputCentricTest : public ConvolutionModule {
 public:
  ...
  void updateOutput(
    ...
    auto filtersCudaTmp = copyToCuda(filtersTmp);
    ...
    auto outputCudaTmp = makeTHCudaTensorFull({
        output.size(0),      // 32
    ...
    bool result =
      InputCentricRelayoutConvolution_UpdateOutput(inputCuda.get(),
                                                   filtersCudaTmp.get(),
                                                   filterRowStride,
                                                   filterColStride,
                                                   outputCudaTmp.get());
    ...
    auto outputTmp = copyFromCuda(outputCudaTmp.get());
...
class CuFFT : public ConvolutionModule {
 public:
  ...
  void updateOutput(
    ...
    auto realComplexPair = makeCuFFTTensors<kFFTDims>(input, maxSizes);
    ...
    auto inputComplexTHCudaTensorT = makeCuFFTTensorComplex<kFFTDims>(
      inputTHCudaTensor.get(), maxSizes);

    realComplexPair =
      makeCuFFTTensors<kFFTDims>(filters, maxSizes);
    ...
    auto filtersComplexTHCudaTensorT = makeCuFFTTensorComplex<kFFTDims>(
      filtersTHCudaTensor.get(), maxSizes);

    realComplexPair =
      makeCuFFTTensors<kFFTDims>(output, maxSizes);
    ...
    auto outputComplexTHCudaTensorT = makeCuFFTTensorComplex<kFFTDims>(
      outputTHCudaTensor.get(), maxSizes);
    ...
    if (impl_ == Implementation::Reference) {
      CuFFTConvolution_ReferenceUpdateOutput(inputTHCudaTensor.get(),
      ...
      CuFFTConvolution_UpdateOutput(inputTHCudaTensor.get(),
                                    filtersTHCudaTensor.get(),
                                    outputTHCudaTensor.get(),
                                    biasCuda.get(),
                                    inputComplexTHCudaTensor.get(),
    ...
    if (FLAGS_verify) {
      checkExpectedInput(input, copyFromCuda(inputTHCudaTensor.get()));
      checkExpectedInput(filters, copyFromCuda(filtersTHCudaTensor.get()));
      // Recover actual output from padded layout, output is smaller
      // than outputTmp when kernelSize > 1
      auto outputTmp = copyFromCuda(outputTHCudaTensor.get());
      for (long i = 0; i < output.size(0); ++i) {
        for (long j = 0; j < output.size(1); ++j) {
          for (long k = 0; k < output.size(2); ++k) {
            for (long l = 0; l < output.size(3); ++l) {
  ...
  void accGradParameters(
    ...
    auto realComplexPair =
      makeCuFFTTensors<kFFTDims>(input, maxSizes);
    auto inputTHCudaTensor = std::move(realComplexPair.first);
    auto inputComplexTHCudaTensor = std::move(realComplexPair.second);
    auto inputComplexTHCudaTensorT = makeCuFFTTensorComplex<kFFTDims>(
      inputTHCudaTensor.get(), maxSizes);
    ...
    if (impl_ == Implementation::Reference) {
      ...
      CuFFTConvolution_AccGradParameters(inputTHCudaTensor.get(),
                                         filtersTHCudaTensor.get(),
                                         outputTHCudaTensor.get(),
                                         biasCuda.get(),
                                         scale,
    ...
    if (FLAGS_verify) {
      checkExpectedInput(input, copyFromCuda(inputTHCudaTensor.get()));
      checkExpectedInput(output, copyFromCuda(outputTHCudaTensor.get()));
      // Recover actual filters from padded layout, filters is smaller
      // than filtersTmp when kernelSize > 1
      auto filtersTmp = copyFromCuda(filtersTHCudaTensor.get());
      for (long i = 0; i < filters.size(0); ++i) {
        for (long j = 0; j < filters.size(1); ++j) {
          for (long k = 0; k < filters.size(2); ++k) {
            for (long l = 0; l < filters.size(3); ++l) {
  ...
  void updateGradInput(
    ...
    auto filtersComplexTHCudaTensorT =
      makeCuFFTTensorComplex<kFFTDims>(filtersTHCudaTensor.get(), maxSizes);
    ...
    if (FLAGS_verify) {
      ...
      // Recover actual filters from padded layout, filters is smaller
      // than filtersTmp when kernelSize > 1
      auto inputTmp = copyFromCuda(inputTHCudaTensor.get());
...
// Input planes must be a multiple of 4, and stride is not supported
// for NYU FFT
constexpr Params kFFTParams(16, 8, 8, 8, 16, 4, 1);

TEST(CudaConvolutionTest, FFT_updateOutput) {
  auto input =
    makeInputTensor(kFFTParams.batch, kFFTParams.inputPlanes,
                    kFFTParams.inputRow, kFFTParams.inputCol);
  auto filters =
    makeFilterTensor(kFFTParams.outputPlanes, kFFTParams.inputPlanes,
                     kFFTParams.filterSize, kFFTParams.filterSize);

  FFTTest().testUpdateOutput(input, filters,
                             kFFTParams.filterStride,
                             kFFTParams.filterStride,
                             folly::none);
}

TEST(CudaConvolutionTest, FFT_updateGradInput) {
  auto output =
    makeOutputTensor(kFFTParams.batch, kFFTParams.outputPlanes,
                     kFFTParams.outputRow(), kFFTParams.outputCol());

  auto filters =
    makeFilterTensor(kFFTParams.outputPlanes,
                     kFFTParams.inputPlanes,
                     kFFTParams.filterSize, kFFTParams.filterSize);

  FFTTest().testUpdateGradInput(output, filters,
                                kFFTParams.filterStride,
                                kFFTParams.filterStride, folly::none);
}

TEST(CudaConvolutionTest, FFT_accGradParameters) {
  auto input =
    makeInputTensor(kFFTParams.batch, kFFTParams.inputPlanes,
                    kFFTParams.inputRow, kFFTParams.inputCol);
  ...
  FFTTest().testAccGradParameters(input, output,
                                  kFFTParams.filterStride,
                                  kFFTParams.filterStride,
                                  1.0f, folly::none);
}
...
TEST(CudaConvolutionTest, CuFFT_updateGradInput_fixed) {
  ...
  CuFFT::checkExpectedInput(expectedInput,
                            copyFromCuda(cufft.saveInputTHCudaTensor.get()));
}
