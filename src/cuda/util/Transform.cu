...
void transform(const typename Op::Input* input,
  ...
  transformKernel<Op><<<gridDim, blockDim>>>(input, out, n);
}

template void transform<ToHalf>(const ToHalf::Input* in, ToHalf::Output* out,
                                size_t n);
template void transform<ToFloat>(const ToFloat::Input* in,
                                 ToFloat::Output* out, size_t n);
