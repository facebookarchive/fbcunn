...
std::pair<std::unique_ptr<THCudaTensor, CudaTensorDeleter>,
  ...
  auto tCuda = copyToCuda(t);
  auto ttCuda = copyToCuda(tt);
  DeviceTensor<float, Dim> tCudaTensor =
    torchToDeviceTensor<float, Dim>(tCuda.get());
  DeviceTensor<float, Dim> ttCudaTensor =
    torchToDeviceTensor<float, Dim>(ttCuda.get());

  transpose(tCudaTensor, ttCudaTensor, sep, asComplex);
  tt = copyFromCuda(ttCuda.get());
...
void unTransposeAndCheckOutOfPlace(
    ...
    bool asComplex = false) {
  auto ct = torchToDeviceTensor<float, Dim>(pCudaTensor.first.get());
  auto ctt = torchToDeviceTensor<float, Dim>(pCudaTensor.second.get());

  transpose(ct, ctt, Dim - sep, asComplex);
  pTensor.second = copyFromCuda(pCudaTensor.first.get());
