 ...
 public:
  void configure(TestConfig config,
    ...
    auto realComplexPair =
      makeCuFFTTensors<FFTDim>(input, FFTSize, cfg.inPlace);
    inputTHCudaTensor = std::move(realComplexPair.first);
    fftTHCudaTensor = std::move(realComplexPair.second);

    inputCudaTensor =
      torchToDeviceTensor<float, FFTDim + BatchDim>(inputTHCudaTensor.get());
    outputCudaTensor =
      torchToDeviceTensor<float, FFTDim + BatchDim + 1>(fftTHCudaTensor.get());
...
TEST_F(FFT2DTest, test2x2ConstantInPlace) {
  ...
  checkExpectedOutput(expected, copyFromCuda(fftTHCudaTensor.get()));
}
...
TEST_F(FFT2DTest, test2x2ConstantInPlacePadded) {
  ...
  checkExpectedOutput(expected, copyFromCuda(fftTHCudaTensor.get()), 5e-5f);
}
...
TEST_F(FFT2DTest, testInverseOutOfPlace) {
  ...
  checkExpectedInput(input, copyFromCuda(inputTHCudaTensor.get()), 5e-4f);
}
...
TEST_F(FFT2DTest, testInverseInPlace) {
  ...
  checkExpectedInput(input, copyFromCuda(inputTHCudaTensor.get()), 5e-5f);
}
