...
makeTHCudaTensorSameSize(const thpp::Tensor<float>& t);
...
template <int Dim>
std::unique_ptr<THCudaTensor, CudaTensorDeleter>
makeTHCudaTensorSameSize(const cuda::DeviceTensor<float, Dim>& t) {
  ...
  return makeTHCudaTensorFull(sizes, strides);
}
