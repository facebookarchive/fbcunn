...
#define MIN_BLOCKS_PER_MULTIPROCESSOR 4
    ...
    for (int outputCol = 0; outputCol < outputCols - 1; ++outputCol) {
      ...
      for (int plane = 0; plane < NumInputPlanes; ++plane) {
        ...
        rotateAndFetch<FilterColSize, FilterColStride>
          (in[plane],
           input[batch][planeMin + plane][inputRow][indCol].data(),
           input.getStride(3));
      }
    }
...
#define KERNEL \
  ...
  const int optTileFilter = std::min(256, MAX_THREADS_PER_BLOCK);
  const int optTileRow = MAX_THREADS_PER_BLOCK / optTileFilter;
...
bool InputCentricRelayoutConvolution_UpdateOutput(THCudaTensor* inputTH,
  ...
  DeviceTensor<float, 4> input = torchToDeviceTensor<float, 4>(inputTH);
  DeviceTensor<float, 4> filters = torchToDeviceTensor<float, 4>(kernelsTH);
  ...
  //   MAX_THREADS_PER_BLOCK = 96
  //   MIN_BLOCKS_PER_MULTIPROCESSOR = 4
