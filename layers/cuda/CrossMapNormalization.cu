...
#include "torch/fb/fbcunn/layers/cuda/CrossMapNormalization.cuh"
...
void launchCrossMapNormalizationUpdateOutputKernel(
  ...
  updateOutput<<<grid, block, smemPerBlock>>>(input, output, squaredSum, 
      param);
}
...
void launchCrossMapNormalizationUpdateGradInputKernel(
  ...
  cudaDeviceSetCacheConfig(cudaFuncCachePreferShared);  
  updateGradInput<<<grid, block, smemPerBlock>>>(input, gradOutput, squaredSum,
      gradInput, param);
}
