...
temporalKMaxPoolingUpdateOutput(DeviceTensor<float, 3> input,
  ...
  DeviceTensor<float, 1> input1d(input[batch][0][feature].data(),
                                 (const int[1]){ input.getSize(1) },
                                 (const int[1]){ input.getSize(2) });
  DeviceTensor<float, 1> output1d(output[batch][0][feature].data(),
                                  (const int[1]){ k },
                                  (const int[1]){ output.getSize(2) });
  DeviceTensor<float, 1> indices1d(indices[batch][0][feature].data(),
                                   (const int[1]){ k },
...
temporalKMaxPoolingUpdateGradInput(DeviceTensor<float, 3> gradOutput,
  ...
  for (int feature = threadIdx.x;
    ...
    atomicAdd(gradInput[batch][index][feature].data(),
...
void
runTemporalKMaxPoolingUpdateOutput(const DeviceTensor<float, 3>& input,
  ...
  temporalKMaxPoolingUpdateOutput<<<grid, block>>>(
    input, indices, output, k);
}

void
runTemporalKMaxPoolingUpdateGradInput(const DeviceTensor<float, 3>& gradOutput,
                                      const DeviceTensor<float, 3>& indices,
                                      DeviceTensor<float, 3>& gradInput,
                                      int k) {
  const cudaDeviceProp& deviceProperties =
  ...
  temporalKMaxPoolingUpdateGradInput<<<grid, block>>>(
