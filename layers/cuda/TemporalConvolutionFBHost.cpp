...
int updateOutput(lua_State *L)
{
  ...
  weight = torchToDeviceTensor<float, 3>(weightTH);
  ...
  bias::updateOutputTemporalBias(outputTH, biasTH);
...
int updateGradInput(lua_State *L) {
  ...
  if (gradOutputDim == 2) {
    ...
    auto tmpIn = torchToDeviceTensor<float, 2>(gradInputTH);
    gradInput = tmpIn.upcastOuter<3>();
  } else {
    CHECK_EQ(3, gradOutputDim);
    gradOutput = torchToDeviceTensor<float, 3>(gradOutputTH);
...
int accGradParameters(lua_State *L) {
  ...
  if (gradOutputDim == 2) {
    auto tmpOut = torchToDeviceTensor<float, 2>(gradOutputTH);
    gradOutput = tmpOut.upcastOuter<3>();
    auto tmpIn = torchToDeviceTensor<float, 2>(inputTH);
  ...
  gradWeight = torchToDeviceTensor<float, 3>(gradWeightTH);
  auto nBatches = input.getSize(0);
  auto inputHeight = input.getSize(1);
  auto outputHeight = gradOutput.getSize(1);
  auto inputWidth = input.getSize(2);
  ...
  bias::accGradParametersTemporalBias(gradOutputTH, gradBiasTH, scale);
