...
#include "THCTensor.h"
#include "torch/fb/fbcunn/layers/cuda/BLASParameters.h"
...
struct matmultBatchedStruct {
  void run(DeviceTensor<float, Dim>& C,
    ...
    auto Cd = C.template downcast<NewDims>();
    auto Ad = A.template downcast<NewDims>();
    auto Bd = B.template downcast<NewDims>();
