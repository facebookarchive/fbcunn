...
void CuFFTConvolution_ReferenceUpdateGradInput(THCudaTensor* inputTH,
  ...
  DeviceTensor<float, 4> weight =
    torchToDeviceTensor<float, 4>(weightTH);
  ...
  DeviceTensor<float, 5> weightComplex =
    torchToDeviceTensor<float, 5>(weightComplexTH);
...
void CuFFTConvolution_UpdateGradInput(THCudaTensor* inputTH,
                                      THCudaTensor* weightTH,
                                      THCudaTensor* outputTH,
                                      THCudaTensor* inputComplexTH,
                                      THCudaTensor* weightComplexTH,
  ...
  conv.withInputAndBuffers(inputTH, inputComplexTH, inputComplexTTH)
    .withFiltersAndBuffers(weightTH, weightComplexTH, weightComplexTTH)
