...
#include "THCTensor.h"
#include "torch/fb/fbcunn/layers/cuda/CuBLASWrapper.h"
#include "torch/fb/fbcunn/layers/cuda/bias/ConvolutionBias.cuh"
#include "torch/fb/fbcunn/layers/cuda/fft/CuFFTWrapper.cuh"
#include "torch/fb/fbcunn/layers/cuda/fft/CuFFTConvolution.cuh"
#include "torch/fb/fbcunn/layers/cuda/fft/Utils.cuh"
...
void
CuFFTConvolution_ReferenceAccGradParameters(THCudaTensor* inputTH,
  ...
  DeviceTensor<float, 4> filters =
    torchToDeviceTensor<float, 4>(gradWeightTH);
  DeviceTensor<float, 4> input =
    torchToDeviceTensor<float, 4>(inputTH);
  DeviceTensor<float, 4> gradOutput =
    torchToDeviceTensor<float, 4>(gradOutputTH);

  DeviceTensor<float, 5> inputComplex =
    torchToDeviceTensor<float, 5>(inputComplexTH);
  DeviceTensor<float, 5> gradOutputComplex =
    torchToDeviceTensor<float, 5>(gradOutputComplexTH);
  DeviceTensor<float, 5> filtersComplex =
    torchToDeviceTensor<float, 5>(gradWeightComplexTH);
  ...
  bias::accGradParametersBias(gradOutputTH, gradBiasTH, scale);
}
...
void CuFFTConvolution_AccGradParameters(THCudaTensor* inputTH,
  ...
  conv.withInputAndBuffers(inputTH, inputComplexTH, inputComplexTTH)
    .withFiltersAndBuffers(gradWeightTH, gradWeightComplexTH,
                           gradWeightComplexTTH)
    .withOutputAndBuffers(gradOutputTH, gradOutputComplexTH,
