...
#include "torch/fb/fbcunn/layers/cuda/CudaTensorUtils.h"
#include "torch/fb/fbcunn/layers/cuda/fft/CuFFTConvolution.h"
#include "torch/fb/fbcunn/layers/cuda/fft/SpatialConvolutionCuFFT.h"
#include "torch/fb/fbcunn/layers/cuda/fft/SpatialConvolutionCuFFTTuner.h"
...
int updateOutputLua(lua_State* L) {
  ...
  THParams thp(input, weight, output, bias);
  ConvolutionPass pass(ConvolutionPass(ConvolutionPass::kUpdateOutput));
  ProblemSizes pbs(thp, pass);
  detail::updateOutputTH(thp,
                         pbs,
                         SpatialConvolutionCuFFTTuner::getBestPerformance(pbs));
...
int accGradParametersLua(lua_State* L) {
  ...
  THParams thp(input, gradWeight, gradOutput, gradBias, scale);
  ConvolutionPass pass(ConvolutionPass(ConvolutionPass::kAccGradParameters));
  ProblemSizes pbs(thp, pass);
  detail::accGradParametersTH(
    thp,
    pbs,
    SpatialConvolutionCuFFTTuner::getBestPerformance(pbs));
...
int explorePerformanceLua(lua_State* L) {
  ...
  auto batches = asVector(batchesTensor, defaultBatches);
  auto filters = asVector(filtersTensor, defaultFilters);
  auto planes = asVector(planesTensor, defaultPlanes);
  auto inputRows = asVector(inputRowsTensor, defaultInputRows);
  auto inputCols = asVector(inputColsTensor, defaultInputCols);
  auto weightRows = asVector(weightRowsTensor, defaultWeightRows);
  auto weightCols = asVector(weightColsTensor, defaultWeightCols);
