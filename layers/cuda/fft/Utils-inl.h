...
#include "torch/fb/fbcunn/layers/Storage.h"
...
long numFFTCols(long cols) {
  return numHermitianSymmetryCols(cols);
}
...
makeCuFFTTensorReal(
  ...
  // enclosing tensor that can fit (commonRows x commonCols) float[2]
  {
    ...
    if (candidateCudaStorageReal == nullptr ||
      ...
      real = makeTHCudaTensorFull(size, stride);
    } else {
      real = makeAliasedTHCudaTensorFull(candidateCudaStorageReal,
                                         size,
...
makeCuFFTTensorComplex(
  ...
  if (inPlace == FFTOutputSpecification::InPlace) {
    complex = makeAliasedTHCudaTensorFull(real, size);
  } else {
    if (candidateCudaStorageComplex == nullptr ||
        *(size.begin()) * *(stride.begin()) >
        candidateCudaStorageComplex->storage->size) {
      complex = makeTHCudaTensorFull(size);
    } else {
      complex = makeAliasedTHCudaTensorFull(candidateCudaStorageComplex,
                                            size,
                                            std::vector<long>(stride)
                                           );
    }
...
makeCuFFTTensors(
    ...
    const std::vector<long>& commonDims,
    FFTOutputSpecification inPlace = FFTOutputSpecification::OutOfPlace) {
  auto p1 = makeCuFFTTensorReal<FFTDim>(in, commonDims, nullptr, inPlace);
  auto p2 =
    makeCuFFTTensorComplex<FFTDim>(p1.get(), commonDims, nullptr, inPlace);
...
makeCuFFTTensors(
  ...
  const std::vector<long>& commonDims,
  FFTOutputSpecification inPlace = FFTOutputSpecification::OutOfPlace) {
  auto th = copyToCuda(in);
  auto res = makeCuFFTTensors<FFTDim>(th.get(), commonDims, inPlace);
