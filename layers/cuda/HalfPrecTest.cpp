...
TEST(HalfPrec, cuda) {
  ...
  halfprec_ToHalf(devFloats.data(), devHalfs.data(), devFloats.size());
  ...
  halfprec_ToFloat(devHalfs.data(), exploded.data(), N);
  float postExpl[N];
...
TEST(HalfPrec, exhaustive) {
  ...
  float devOut[N];
  halfprec_ToFloat(devHalfs.data(), devFloats.data(), N);
