...
void runSparseNLLCriterion_updateOutput(
  ...
  updateOutput<<<blocks, threads, sharedSize>>>(targetIdx, targetP, input,
                                                output, batchSize, K);
}
...
void runSparseNLLCriterion_updateGradInput(
  ...
  cudaMemset(gradInput.data(), 0, nClasses * batchSize * sizeof(float));
  ...
  updateGradInput<<<blocks, threads>>>(targetIdx, targetP, gradInput,
                                       batchSize, K);
}
