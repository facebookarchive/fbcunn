...
#include "torch/fb/fbcunn/layers/cuda/FeatureLPPooling.cuh"
...
#pragma unroll
    ...
    // Load output (f(x_is)), but add a bias to make sure it isn't
    // zero
    float out = output[batch][outputFeature][dim1Point][dim2Point];
    if (out == 0.0f) {
      // really small but still a normalized float
      out = 1e-35f;
    }
    ...
    if (curStartInputFeature >= exclusiveStartInputFeature &&
      ...
      for (int i = 0; i < Width; ++i) {
        ...
        if (inputFeature >= exclusiveStartInputFeature &&
          ...
          atomicAdd(gradInput[batch][inputFeature][dim1Point][dim2Point].data(),
                    val);
        }
      }
    }
...
bool
runFeatureLPPoolingUpdateOutput(const DeviceTensor<float, 4>& input,
                                DeviceTensor<float, 4>& output,
...
bool
runFeatureLPPoolingUpdateGradInput(const DeviceTensor<float, 4>& gradOutput,
                                   const DeviceTensor<float, 4>& input,
                                   const DeviceTensor<float, 4>& output,
                                   DeviceTensor<float, 4>& gradInput,
                                   float power, int width, int stride) {
